...
<div class="explain">
<b>テーマ名はどう付けたか（と、よくある誤解）。</b>
NMFは「訴訟×語」の行列を <b>W（訴訟ごとのテーマ配合）</b> と <b>H（テーマごとの語の重み）</b> の2つに分解します。
各テーマの中身はHで重みが高い語で決まります。ここで大事なのは、
<b>7,350語を6グループに“排他的に仕分け”しているのではない</b>こと——各語は6テーマすべてに重み（0以上）を持ち、
「収容」のように複数テーマで高い語もあります。カードの特徴語は各テーマで<b>重みが上位の語を表示しているだけ</b>で、
同じ語が複数テーマに現れることもあります。
<span class="note">補足: そのうえで<b>テーマ名（「刑事手続」等）は、高重みの語と代表ケースを人が読んで付けた要約ラベル</b>です
